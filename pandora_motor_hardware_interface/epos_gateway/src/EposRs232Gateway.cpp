...
EposRs232Gateway::EposRs232Gateway(void){
	//gateway not initialized since to RS232 port, baud rate and timeout is was
	//not provided
	initialized=false;
...
EposRs232Gateway::EposRs232Gateway(const std::string& device,unsigned int baudRate,unsigned int timeout){
	//gateway is not initialized until we actually connect to the RS232 port
	initialized=false;
	//initialize POSIX thread mutex, for use during read/write operations
	pthread_mutex_init(&gatewayMutex,NULL);
	
	//connect to the RS232 port using port Rs232 object
	int status=this->port.open(device,baudRate,timeout);
	
	if(!status){
		//failed to connect to serial port
		std::cerr<<"error: EposRs232Gateway: cannot open serial port"<<std::endl;
		return;
	}
...
epos::CommandStatus EposRs232Gateway::getACK(void){
...
epos::CommandStatus EposRs232Gateway::ACK(void){
...
epos::CommandStatus EposRs232Gateway::sendFrame(unsigned char opCode,epos::Word *data,unsigned short length,epos::Word *response){
...
epos::CommandStatus EposRs232Gateway::readObject(unsigned char nodeID,uint16_t index,unsigned char subIndex,epos::DWord *responce){}

epos::CommandStatus EposRs232Gateway::readObject(unsigned char nodeID,uint16_t index,unsigned char subIndex,char *responce){}

epos::CommandStatus EposRs232Gateway::readObject(unsigned char nodeID,uint16_t index,unsigned char subIndex,epos::Word *responce){
	epos::Word data[2];
	data[0]=index;
...
epos::CommandStatus EposRs232Gateway::writeObject(unsigned char nodeID,uint16_t index,unsigned char subIndex,epos::DWord data){
...
epos::CommandStatus EposRs232Gateway::writeObject(unsigned char nodeID,uint16_t index,unsigned char subIndex,epos::Word data){
	epos::Word internalData[4];
	internalData[0]=index;
	internalData[1]=(nodeID<<8)|subIndex;
	internalData[2]=data;
...
epos::CommandStatus EposRs232Gateway::writeObject(unsigned char nodeID,uint16_t index,unsigned char subIndex,char data){
	epos::Word internalData[4];
...
epos::Word EposRs232Gateway::crc16CCITT(epos::Word* data,unsigned int length){
